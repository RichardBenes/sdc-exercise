...
To speed up the processing, the app uses multiple threads:

* Main thread reads the XLSX file, parses out cells, and puts them to a queue
* Worker threads (`PrimalityChecker`):
** read cells from the queue and check the values & their primality
** put the computed values to Outputter's queue
* `Outputter` thread 
** sorts the computted values (into the order in which they occured in the input file) 
** and prints them to the screen

At the end, the Main thread `join()`s all the worker threads.
...
So currently, I'm pretty much "just" adding another `Appender` to the Log4J2 config, which captures the log messages (only the `%m` part though) in memory, so I could check that later.
...
I tried running the `AKSTest#testAKS` in parallel (jUnit allows that), but it didn't bring any signigicant time saving. 
